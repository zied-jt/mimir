...
type CircuitBreakerConfig struct {
	...
	PercentageAllowedRequestsInOpenState uint          `yaml:"percentage_allowed_requests_in_open_state" category:"experimental"`
}
...
func (cfg *CircuitBreakerConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.UintVar(&cfg.PercentageAllowedRequestsInOpenState, prefix+".circuit-breaker.percentage-allowed-requests-in-open-state", 0, "Percentage of requests that will be allowed when the circuit breaker is in the open state. If it is different than 0, the circuit breaker will never wait before allowing new requests.")
}
