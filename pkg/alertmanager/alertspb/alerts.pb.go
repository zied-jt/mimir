...
type AlertConfigDesc struct {
	...
	GrafanaTemplates []*TemplateDesc `protobuf:"bytes,4,rep,name=grafana_templates,json=grafanaTemplates,proto3" json:"grafana_templates,omitempty"`
}
...
func (m *AlertConfigDesc) GetGrafanaTemplates() []*TemplateDesc {
	if m != nil {
		return m.GrafanaTemplates
	}
	return nil
}
...
var fileDescriptor_20493709c38b81dc = []byte{
	...
	,
}
...
func (this *AlertConfigDesc) Equal(that interface{}) bool {
	...
	if len(this.GrafanaTemplates) != len(that1.GrafanaTemplates) {
		return false
	}
	for i := range this.GrafanaTemplates {
		if !this.GrafanaTemplates[i].Equal(that1.GrafanaTemplates[i]) {
			return false
		}
	}
	return true
...
func (this *AlertConfigDesc) GoString() string {
	...
	if this.GrafanaTemplates != nil {
		s = append(s, "GrafanaTemplates: "+fmt.Sprintf("%#v", this.GrafanaTemplates)+",\n")
	}
	s = append(s, "}")
...
func (m *AlertConfigDesc) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if len(m.GrafanaTemplates) > 0 {
		for iNdEx := len(m.GrafanaTemplates) - 1; iNdEx >= 0; iNdEx-- {
			{
				size, err := m.GrafanaTemplates[iNdEx].MarshalToSizedBuffer(dAtA[:i])
				if err != nil {
					return 0, err
				}
				i -= size
...
func (m *AlertConfigDesc) Size() (n int) {
	...
	if len(m.GrafanaTemplates) > 0 {
		for _, e := range m.GrafanaTemplates {
			l = e.Size()
			n += 1 + l + sovAlerts(uint64(l))
		}
	}
...
func (this *AlertConfigDesc) String() string {
	...
	repeatedStringForGrafanaTemplates := "[]*TemplateDesc{"
	for _, f := range this.GrafanaTemplates {
		repeatedStringForGrafanaTemplates += strings.Replace(f.String(), "TemplateDesc", "TemplateDesc", 1) + ","
	}
	repeatedStringForGrafanaTemplates += "}"
	s := strings.Join([]string{`&AlertConfigDesc{`,
		`User:` + fmt.Sprintf("%v", this.User) + `,`,
		`RawConfig:` + fmt.Sprintf("%v", this.RawConfig) + `,`,
		`Templates:` + repeatedStringForTemplates + `,`,
		`GrafanaTemplates:` + repeatedStringForGrafanaTemplates + `,`,
		`}`,
	}, "")
	return s
}
...
func (m *AlertConfigDesc) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field GrafanaTemplates", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.GrafanaTemplates = append(m.GrafanaTemplates, &TemplateDesc{})
			if err := m.GrafanaTemplates[len(m.GrafanaTemplates)-1].Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
